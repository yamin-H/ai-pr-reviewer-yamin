  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center glass-card max-w-lg mx-auto mt-12 p-8 border border-red-500/10 bg-red-500/5">
        ...
        <p className="text-sm text-zinc-400 mb-6">{error || "Could not retrieve settings data."}</p>
  ...
  return (
    <div className="space-y-10 max-w-5xl">
      ...
      {org.isInstalled === false && (
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-5 text-red-200 shadow-lg">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              <AlertTriangle className="h-5 w-5 text-red-400 shrink-0 mt-0.5" />
              <div>
                <h3 className="text-sm font-semibold text-red-300">GitHub App is Uninstalled</h3>
      ...
      <Card className="border border-white/[0.08] bg-[#0A0E18]">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <CardTitle className="text-base font-bold text-white">Organization Profile</CardTitle>
        ...
        <CardContent className="pt-2">
          <div className="grid gap-4 sm:grid-cols-4 border border-white/[0.06] rounded-xl p-4 bg-white/[0.01]">
            <div>
              <p className="text-[11px] text-zinc-500 font-medium uppercase tracking-wider">GitHub Organization</p>
              <p className="text-sm font-semibold text-white mt-1">@{org.login}</p>
            </div>
            <div>
              <p className="text-[11px] text-zinc-500 font-medium uppercase tracking-wider">App Installation ID</p>
              <p className="text-sm font-mono text-indigo-300 mt-1">{org.installationId}</p>
            </div>
            <div>
              <p className="text-[11px] text-zinc-500 font-medium uppercase tracking-wider">App Status</p>
              <p className={`text-sm font-semibold mt-1 ${org.isInstalled === false ? "text-red-400" : "text-emerald-400"}`}>
        ...
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <CardTitle className="text-base font-bold text-white">Review Sensitivity & Automation</CardTitle>
                <CardDescription className="text-xs text-zinc-400">
            ...
            {orgSaveSuccess && (
              <span className="inline-flex items-center gap-1.5 text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded-lg">
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border p-4 text-left transition-all cursor-pointer ${
                      isSelected
                        ? "border-indigo-500 bg-indigo-500/10 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1)]"
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 flex items-center justify-between gap-4">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                  triggerOnSync ? "bg-indigo-600" : "bg-zinc-700"
                }`}
              >
                <span
      ...
      <Card className="border border-white/[0.08] bg-[#0A0E18]">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <CardTitle className="text-base font-bold text-white">Repository Controls</CardTitle>
              <CardDescription className="text-xs text-zinc-400">
        ...
        <CardContent className="pt-2">
          {repos.length === 0 ? (
            ...
            <div className="divide-y divide-white/[0.06] border border-white/[0.06] rounded-xl overflow-hidden bg-white/[0.01]">
              {repos.map((repo) => {
                ...
                return (
                  <div
                    key={repo.id}
                    className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 hover:bg-white/[0.02] transition-colors"
                  >
                    <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-3 self-end sm:self-center">
                      <Button
                        ...
                        className="h-7 text-[11px] gap-1.5 px-2.5 text-zinc-300 hover:text-white border-white/[0.08]"
                      >
      ...
      <Card className="border border-white/[0.08] bg-[#0A0E18]">
        <CardHeader className="pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <CardTitle className="text-base font-bold text-white">Custom Rule Sets (.powerful.yml)</CardTitle>
              <CardDescription className="text-xs text-zinc-400">
        ...
        <CardContent className="space-y-4 pt-1">
          <p className="text-xs text-zinc-300 leading-relaxed">
            Commit a <code className="font-mono text-indigo-300 bg-indigo-500/10 px-1.5 py-0.5 rounded border border-indigo-500/20">.powerful.yml</code> file in the root directory of any connected repository. The LangGraph agent parses these instructions during the review pipeline and strictly enforces them across every code chunk.
          </p>

          <div className="rounded-xl border border-white/[0.08] bg-black/40 p-4 font-mono text-xs text-zinc-300 space-y-1 overflow-x-auto">
