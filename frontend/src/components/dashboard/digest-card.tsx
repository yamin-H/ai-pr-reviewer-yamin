...
export function DigestCard({ digest }: { digest: WeeklyDigest }) {
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="border-b border-zinc-200/80 dark:border-white/[0.04] bg-zinc-50/50 dark:bg-white/[0.02]">
      ...
      <CardContent className="p-6">
        ...
        <div className="mb-4 flex items-center gap-2">
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-zinc-200/80 dark:bg-white/5">
            <div
...
function InsightRow({
  ...
  return (
    <div className="flex gap-3 rounded-xl bg-zinc-50 dark:bg-white/[0.02] border border-zinc-200/60 dark:border-transparent p-3">
      <Icon className={`mt-0.5 h-4 w-4 shrink-0 ${accent}`} />
      <div>
        <p className="text-xs font-medium text-zinc-500">{label}</p>
        <p className="text-sm text-zinc-800 dark:text-zinc-300">{value}</p>
      </div>
    </div>
  );
}
